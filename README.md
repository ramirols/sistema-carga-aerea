...
- **Gestión de vuelos**: registro, edición, cambio de estado (Programado/Despachado/Cancelado), búsqueda y filtros por fecha y estado. No se permite programar un vuelo con fecha/hora de salida en el pasado. Tres procesos programados corren solos: uno despacha automáticamente los vuelos cuya hora de salida ya pasó, otro marca como "Arribada" las encomiendas embarcadas cuyo vuelo ya despachado ya llegó a destino, y otro declara en abandono las que nadie recogió dentro del plazo configurado tras arribar. Al cancelar un vuelo, las encomiendas que tenía asignadas (embarcadas o en almacén) se liberan automáticamente; también se puede desembarcar una encomienda de un vuelo que aún no fue despachado.
- **Gestión de encomiendas**: registro con código de seguimiento generado automáticamente, cálculo de **peso volumétrico** (largo × ancho × alto ÷ 5000), asignación a vuelos con validación de capacidad disponible (embarque automático), cambio de estado y búsqueda con filtros. Estados: En almacén, Embarcada, **Arribada** (el vuelo ya llegó y está disponible para que la recojan, detectado automáticamente por otro proceso programado), Entregada, Cancelada y Abandonada (cuando nadie la recoge dentro del plazo configurado tras arribar). La entrega exige registrar quién recibió la carga (nombre y DNI), validando que coincida con el destinatario original o con la persona/empresa autorizada y aprobada.
- **Rastreo público**: página `/rastreo`, sin necesidad de iniciar sesión, donde cualquier persona con el código de una encomienda puede consultar su estado a través de un asistente conversacional (Valeria), incluyendo remitente, destinatario, estado del vuelo (Programado/Despachado), fecha/hora de llegada, aeropuerto y terminal de carga de destino, y el estado de cualquier solicitud de cambio de consignatario en curso.
- **Cambio de consignatario**: página pública `/consignatario` donde el destinatario original (verificado por su DNI) autoriza a otra persona o empresa a recoger su encomienda, adjuntando una carta poder firmada (imagen o PDF, subida a Cloudinary con entrega firmada/segura) y, opcionalmente, un contacto para que el staff pueda comunicarle el resultado. Cada solicitud pasa por un flujo de aprobación (Pendiente/Aprobada/Rechazada) desde el módulo interno **Peticiones**, que muestra en paralelo el registro original de la encomienda (a partir del historial de auditoría) y la solicitud de cambio, con buscador, filtro por estado, verificación directa por código de seguimiento, vista de lista o tarjetas, visor de la carta poder sin necesidad de descargarla y opción de eliminar la autorización.
...
- **Auditoría y recuperación**: historial completo de cambios (creación, modificación, eliminación) sobre vuelos, encomiendas y usuarios, con filtros por entidad, operación, usuario y rango de fechas, comparación campo por campo entre versiones y la posibilidad de restaurar una versión anterior de cualquier registro. Implementado con Hibernate Envers.
...
- **Notificaciones de error centralizadas**: cualquier error del backend (validaciones, reglas de negocio, permisos) se muestra automáticamente como notificación en toda la aplicación, sin necesidad de manejarlo pantalla por pantalla.
- **Modo oscuro** (en el panel interno) y diseño responsive (escritorio, tablet y celular).
...
- Maven Wrapper
- MySQL
- Cloudinary (almacenamiento seguro de la carta poder del cambio de consignatario)
...
Luego abre `http://localhost:4200` en el navegador e inicia sesión. El backend crea automáticamente un usuario administrador la primera vez que arranca (ver `DataInitializer`); revisa esa clase para conocer las credenciales iniciales, ya que no se documentan aquí por seguridad.

Para probar el rol `OPERADOR`, inicia sesión como administrador, ve a **Usuarios → Nuevo usuario** y crea uno con ese rol.
